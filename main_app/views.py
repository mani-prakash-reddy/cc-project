...
def predict(task_name, image_path):
    ...
    return result.get('result', 'Unknown Class')


def cd(request):
    if request.method == 'POST':
        # Assuming your file input name is 'image'
        uploaded_image = request.FILES['image']

        # Save the uploaded image temporarily (you may want to handle this better)
        with open('temp.jpg', 'wb+') as destination:
            for chunk in uploaded_image.chunks():
                destination.write(chunk)

        # Make a prediction using the uploaded image
        task_name = request.POST.get('task_name')
        prediction = predict(task_name, 'temp.jpg')

        # Pass the prediction to the template
        return render(request, 'patient/checkdisease/cd.html', {'prediction': prediction})

    return render(request, 'patient/checkdisease/cd.html')
